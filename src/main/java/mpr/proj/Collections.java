...
public abstract class Collections {
	...
	public static Set<Country> getCountries()	{
		try	{
			...
			ResultSet rs = stmt.executeQuery(queryStr);
			while(rs.next())	{
				dane.add(new Country(rs.getLong("ID"),rs.getString("NAME"),rs.getString("CODE")==null ? "Nieznany" : rs.getString("CODE")));
			}
			return dane;
		}
		catch (Exception ex)	{
	...
	public static Set<String> getSex()	{
		try	{
			Set<String> dane = new HashSet<String>();
			String queryStr = "SELECT * FROM SEX";
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery(queryStr);
			while(rs.next())	{
				if(rs.getLong("ID")==0)	{
